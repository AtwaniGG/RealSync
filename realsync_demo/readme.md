...
5. **Video deepfake scoring** (placeholder for demo)
...
│  ├─ video_model.py         # Deepfake detection
...
- Uses `tensorflow==2.15.0` for macOS Apple Silicon
...
{'video_score': 0.65, 'emotion_score': 0.5, 'audio_score': 0.0, 'trust_score': 0.53}
```
...
- **video_score** (0-1): Deepfake probability score
  - Higher = more likely to be manipulated
  - Current: placeholder with random values (0.4-0.9)
...
## 10. What You Have After Finishing
...
3. **Emotion Analysis** (`emotion_model.py`)
...
4. **Audio Processing** (`audio_extract.py`)
   - Extracts audio track from video
   - Splits into 5-second chunks
   - Saves as WAV files

5. **Scoring** (`run_pipeline.py`)
